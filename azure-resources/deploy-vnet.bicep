...
param aciSubnetName string = '${vnetName}-aci-subnet'
@description('aci subnet address prefix')
param aciPrefix string = '10.253.0.0/16'
@allowed([
  'Enabled'
  'Disabled'
])
@description('private endpoint network pocilies of aci subnet')
param aciEndpointPolicy string = 'Enabled'
@allowed([
  'Enabled'
  'Disabled'
])
@description('private link service pocilies of aci subnet')
param aciServicePolicy string = 'Enabled'
...
var subnets = [
  ...
  }
  {
    name: aciSubnetName
    prefix: aciPrefix
    endpointPolicy: aciEndpointPolicy
    servicePolicy: aciServicePolicy
    delegations: [
      {
        name: 'aciDelegation'
        properties: {
